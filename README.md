...
Um incidente é criado ou atualizado na Instância A.
Uma Business Rule dispara automaticamente e chama o Script Include ApiAmigo.
O Script Include verifica se esse incidente já foi enviado antes (via campo customizado u_sys_id_remoto):
...
Se houver comentário ou work note novo, ele é identificado e incluído no payload.
Na primeira criação, o sys_id do incidente remoto retornado pela API é salvo no campo u_sys_id_remoto do incidente local — essa referência é o que permite que atualizações futuras apontem para o registro certo.
...
javascriptif (current.u_integrado == true) {
    return;
}
...
Projeto desenvolvido em dupla por Lucas Borges e um colega, como prática de integração real entre instâncias ServiceNow, durante transição de carreira de Logística para Tecnologia.
